...
import utils
from utils import PGD_Linf, LipschitzEstimator
...
def main(args: argparse.Namespace):
    ...
    mdd = MarginDisparityDiscrepancy(4.0).to(device)
    ...
    # define attack
    attack_train = PGD_Linf(classifier, epsilon=args.eps, step_size=args.step_size, num_steps=args.train_numsteps, random_start=True,
    ...
    if args.phase == 'test':
        ...
        print("rob_acc1 = {:3.2f}".format(rob_acc1))
        print("aa_acc1 = {:3.2f}".format(aa_acc1))
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.warmup:
            ...
            teacher_mdd = MarginDisparityDiscrepancy(4.0).to(device)
            ...
            optimizer = SGD(classifier.get_parameters(),
                    0.005, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
            lr_scheduler = LambdaLR(optimizer, lambda x: 0.005 * (1. + args.lr_gamma * float(x)) ** (-args.lr_decay))
            
            optimizer_teacher = SGD(teacher.get_parameters(),
                    0.005, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
            lr_scheduler_teacher = LambdaLR(optimizer_teacher, lambda x: 0.005 * (1. + args.lr_gamma * float(x)) ** (-args.lr_decay))
            ...
            train_teacher(train_source_iter, train_target_iter, 
                          classifier, teacher, teacher_mdd,
                          optimizer_teacher, lr_scheduler_teacher,
                          epoch, args)
            
            train_rob(train_source_iter, train_target_iter, 
              classifier, mdd, attack_train, teacher,
        ...
        #  model: ImageClassifier, attack_train: PGD_Linf,
        ...
        estimator = LipschitzEstimator(classifier, epsilon=args.eps, n_steps=10, step_size_factor=4, device=device)
        average_lipschitz_constant = estimator.estimate_batch_lipschitz(val_loader)
        print(f"Estimated average Lipschitz constant: {average_lipschitz_constant}")
...
def train_rob(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          classifier: ImageClassifier, mdd: MarginDisparityDiscrepancy, attack_train: PGD_Linf,
    ...
    for i in range(args.iters_per_epoch):
        ...
        pseudo_labels = teacher(normalizer(x_t)).argmax(dim=1).detach()
        
        x_t_adv, _ = attack_train.perturb(x_t, pseudo_labels)
        ...
        outputs, outputs_aux = classifier(x)
        y_s, y_t = outputs.chunk(2, dim=0)
        y_s_aux, _ = outputs_aux.chunk(2, dim=0)

        y_t_adv, y_t_adv_aux = classifier(x_t_adv)
        
        cls_loss = F.cross_entropy(y_s, labels_s, label_smoothing=0.0) 
        lip_loss = F.cross_entropy(y_t_adv, pseudo_labels, label_smoothing=0.0)
        #cls_loss = F.cross_entropy(y_s, labels_s, label_smoothing=0.0) + F.cross_entropy(y_t, pseudo_labels, label_smoothing=0.0)
        transfer_loss = -mdd(y_s, y_s_aux, y_t_adv, y_t_adv_aux)

        loss = (cls_loss + transfer_loss) * args.trade_off + lip_loss
        classifier.step()
...
def train_teacher(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          classifier : ImageClassifier,
          teacher : ImageClassifier, teacher_mdd: MarginDisparityDiscrepancy,
          optimizer: SGD,
          lr_scheduler: LambdaLR, epoch: int, args: argparse.Namespace):
    batch_time = AverageMeter('Time', ':3.1f')
    data_time = AverageMeter('Data', ':3.1f')
    ...
    for i in range(args.iters_per_epoch):
        ...
        #num_adv = int(labels_s.size(0)/2)
        # compute output
        
        
        classifier.eval()
        teacher.eval()
        pred_classifier = classifier(normalizer(x_t)).argmax(dim=1).detach()
        pred_teacher = teacher(normalizer(x_t)).argmax(dim=1).detach()
        select_idx = (pred_classifier != pred_teacher)
        #print(select_idx)
        classifier.train()
        teacher.train()
        ...
        x = torch.cat((x_s, x_t), dim=0)
        outputs, outputs_aux = teacher(x)
        y_s, y_t = outputs.chunk(2, dim=0)
        y_s_aux, y_t_aux = outputs_aux.chunk(2, dim=0)
        ...
        if y_t_aux.size(0) == 0:
            ...
            transfer_loss = -teacher_mdd(y_s, y_s_aux, y_t, y_t_aux)
        
        loss = cls_loss + args.beta * transfer_loss 
        cls_acc = accuracy(y_s, labels_s)[0]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='TARoT for Adversarially Robust Unsupervised Domain Adaptation')
    # dataset parameters
    ...
    parser.add_argument('--trade-off', default=1.0, type=float,
                        help='the trade-off hyper-parameter for robust loss')
