...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.warmup:
            ...
            optimizer = SGD(classifier.get_parameters(), #+ classifier_source.get_parameters(),
                    0.005, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
            optimizer_teacher = SGD(teacher.get_parameters(), #+ classifier_source.get_parameters(),
                    0.005, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
            ...
            train_rob(train_source_iter, train_target_iter, 
              classifier, teacher, attack_train,
              optimizer, optimizer_teacher,
              lr_scheduler, lr_scheduler_teacher, epoch, args)
        else:    
            train_rob(train_source_iter, train_target_iter, 
              classifier, teacher, attack_train,
              optimizer, optimizer_teacher,
...
def train_rob(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          classifier: ImageClassifier, teacher: ImageClassifier, attack_train: PGD_Linf,
          optimizer: SGD, optimizer_teacher: SGD,
          lr_scheduler: LambdaLR, lr_scheduler_teacher: LambdaLR,
    ...
    for i in range(args.iters_per_epoch):
        ...
        teacher.eval()
        y_t_teacher = teacher(normalizer(x_t))
        soft_pseudo_labels = torch.softmax(y_t_teacher, dim=1).detach()
        pseudo_labels = y_t_teacher.argmax(dim=1).detach()
        teacher.train()
        ...
        y_t_adv, _ = classifier(x_t_adv)
        
        loss = F.cross_entropy(y_t_adv, pseudo_labels, label_smoothing=0.0)
        
        y_s, _ = classifier(x_s)
        ...
        with torch.no_grad():
            y_s_new, _ = classifier(x_s)
        new_loss = F.cross_entropy(y_s_new.detach(), labels_s)
        
        dot_product = new_loss - old_loss
        y_t_teacher, _ = teacher(x_t)
        
        t_loss_3 = dot_product * F.cross_entropy(y_t_teacher, pseudo_labels)
        ...
        y_s_teacher, _ = teacher(x_s)
            
        t_loss_1 = F.cross_entropy(y_s_teacher, labels_s)    
            
        t_loss = t_loss_1 + t_loss_2 + t_loss_3 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre_eps', default=1.0, type=float, help='perturbation budget of pretrained adversarially robust model (pre-epsilon)')
    ...
    parser.add_argument('--alpha', default = 2000.0, type = float, 
