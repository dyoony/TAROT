...
from .other_utils import InputNormalize
...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        logits = self.predict(self.normalizer(x))
    ...
    def perturb(self, x, y=None):
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    ...
                    output = self.predict(self.normalizer(x))
        ...
        if not self.targeted:
            #acc = self.predict(x)s.max(1)[1] == y
            acc = self.predict(self.normalizer(x)).max(1)[1] == y
        else:
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict(self.normalizer(adv_curr))
