...
from autoattack.other_utils import L0_norm, L1_norm, L2_norm, InputNormalize
...
class APGDAttack():
    ...
    def __init__(
        ...
        self.y_target = None
        self.normalizer = InputNormalize(torch.tensor([0.485, 0.456, 0.406]).to(device), torch.tensor([0.229, 0.224, 0.225]).to(device))
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'Linf':
            ...
            #print((x_adv - x).shape, (x_adv - x)[0].max()) != 0
        elif self.norm == 'L2':
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    logits = self.model(self.normalizer(x_adv)) # add normalization step
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            y_pred = self.model(self.normalizer(x)).max(1)[1]
        else:
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    #print((adv_curr - x).shape, (adv_curr - x).max()) = 0.0118
                    #print((adv_curr - x).shape, (adv_curr - x).max())
                    #print(acc_curr)
                    #print(x.shape)= [1,3, 224, 224]
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, y=None, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            #y_pred = self.model.predict(x).max(1)[1]
            y_pred = self.model.predict(self.normalizer(x)).max(1)[1]
        if y is None:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        ...
                        output = self.model(self.normalizer(x_to_fool))
                    else:
                        #output = self.model.predict(x_to_fool)
                        output = self.model.predict(self.normalizer(x_to_fool))
