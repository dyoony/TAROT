...
class ConfusionMatrix(object):
    ...
    def format(self, classes: list):
        ...
        std_val = acc.std(unbiased=False)
        # Bottom 10% indices and values
        num_classes = acc.numel()
        k = max(1, int(num_classes * 0.1))  # bottom 10%
        bottom_vals, bottom_indices = torch.topk(acc, k, largest=False)
        bottom_mean = bottom_vals.mean()

        # Output
        print(f" Smallest Value: {min_val.item():.4f} (Class {acc.argmin().item()})")
        print(f" Bottom 10% ({k} classes):")
        for idx, val in zip(bottom_indices.tolist(), bottom_vals.tolist()):
            print(f"  - Class {idx}: {val:.4f}")
        print(f" Mean of Bottom 10%: {bottom_mean.item():.4f}")
        print(f" Max Value: {max_val.item():.4f}")
        print(f" Min Value: {min_val.item():.4f}")
        print(f" Max - Min: {value_range.item():.4f}")
        ...
        return 'global correct: {:.2f}\nmean correct:{:.2f}\nmean IoU: {:.2f}'.format(
            acc_global.item() * 100, acc.mean().item() * 100, iu.mean().item() * 100)
