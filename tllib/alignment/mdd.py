...
class MarginDisparityDiscrepancy(nn.Module):
    ...
    def forward(self, y_s: torch.Tensor, y_s_adv: torch.Tensor, y_t: torch.Tensor, y_t_adv: torch.Tensor,
        ...
        source_loss = -self.margin * self.source_disparity(y_s, y_s_adv) # source_disparity : cross_entropy(reduction=none)
        target_loss = self.target_disparity(y_t, y_t_adv) # target_disparity : log likelihood(reduction=none)
        if w_s is None:
